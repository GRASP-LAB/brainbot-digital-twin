...
static int check_dimension(const char* matrix_name,
                           const char* axis,
                           int actual,
                           int expected) {
  if (actual != expected) {
    // Optional for Arduino:
...
int tiny_setup(TinySolver** solverp,
    ...
    *solverp = solver;
    ...
    // Initialize cache
    status = tiny_precompute_and_set_cache(cache, Adyn, Bdyn, work->Q.asDiagonal(), work->R.asDiagonal(), nx, nu, rho, 0);//verbose);
    if (status) {
        return status;
    }
